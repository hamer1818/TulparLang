...
bool Bvh::build(Arena &arena, const Aabb *item_bounds, uint32_t item_count, uint32_t leaf_threshold) {
  ...
  if (!nodes_ || !item_order_) return false;
...
uint32_t Bvh::build_recursive(const Aabb *item_bounds, uint32_t *order, uint32_t start, uint32_t end,
  ...
  // En genis eksende ortanca-bolme.
  const Vec3 ext = extent(bounds);
  int axis = 0;
  float best_extent = ext.x;
  if (ext.y > best_extent) { axis = 1; best_extent = ext.y; }
  if (ext.z > best_extent) { axis = 2; best_extent = ext.z; }

  const uint32_t mid = start + count / 2;
  std::nth_element(order + start, order + mid, order + end, [&](uint32_t a, uint32_t b) {
