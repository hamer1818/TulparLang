...
static void jit_emit_prologue(JITCompilerState *state) {
    ...
    int stack_top_offset = static_cast<int>(offsetof(VM, stack_top));
    ...
    int stack_offset = static_cast<int>(offsetof(VM, stack));
...
JITCompiledFunc *jit_compile_function(VM *vm, ObjFunction *func) {
    ...
    state.bc_to_native =
        static_cast<size_t *>(calloc(state.bc_length, sizeof(size_t)));
    ...
    // Create result
    JITCompiledFunc *result = static_cast<JITCompiledFunc*>(malloc(sizeof(JITCompiledFunc)));
    if (!result) {
        free(state.bc_to_native);
        jit_code_free(state.code);
        return nullptr;
