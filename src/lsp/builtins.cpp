...
const BuiltinEntry kBuiltins[] = {
    ...
    // Ucunun de istemi ISTEGE BAGLI ve ayni kalipta: arguman str ise ekrana
    // basilir (fflush'lu), degilse sessiz okunur. `input` 2026-09-16'ya kadar
    // istemi hic almiyordu; imza da burada 0-argumanli yaziliydi.
    {"input",        "input(prompt?: str): str",                    "İstemi (varsa) basar, stdin'den bir satır okur."},
    {"input_int",    "input_int(prompt?: str): int",                "İstemi (varsa) basar, stdin'den sayı okur."},
    {"input_float",  "input_float(prompt?: str): float",            "İstemi (varsa) basar, stdin'den ondalıklı sayı okur."},
...
#include "engine_builtins.inc"
    ...
    {"tm_window_resizable", "tm_window_resizable(on)",                  "Pencere boyutlandırılabilir olsun mu. window()'DAN ÖNCE çağrılmalı. Sarmalayıcı: window_resizable()."},
    {"tm_fullscreen",   "tm_fullscreen(on)",                            "Tam ekrana geç / çık (masaüstünde kenarlıksız pencere). Sarmalayıcı: fullscreen()."},
    {"tm_is_fullscreen", "tm_is_fullscreen(): bool",                    "Tam ekranda mı. Sarmalayıcı: is_fullscreen()."},
    {"tm_maximize",     "tm_maximize(on)",                              "Pencereyi büyüt / eski boyuta döndür. Sarmalayıcı: maximize_window()."},
    {"tm_window_resized", "tm_window_resized(): bool",                  "Pencere BU KARE boyutlandırıldı mı. Sarmalayıcı: window_resized()."},
    ...
    {"tm3_billboard_pro", "tm3_billboard_pro(tex: int, x, y, z, size, rot, frame: int, color: int)", "Döndürülebilir + atlas kareli billboard. rot DERECE, dönme merkezi dörtgenin ortası. frame < 0 → dokunun tamamı; >= 0 ise tm3_atlas_grid ile verilen ızgaranın o karesi. Sarmalayıcı: billboard_pro3d()."},
    {"tm3_atlas_grid",  "tm3_atlas_grid(tex: int, cols: int, rows: int)", "Dokuyu cols×rows'luk atlas olarak işaretler. Izgara ÇİZİMİN değil DOKUNUN özelliği, bir kez söylenir. Sarmalayıcı: atlas_izgara3d()."},
    {"tm3_atlas_frames","tm3_atlas_frames(tex: int): int",             "Atlastaki toplam kare sayısı (cols*rows); ızgara verilmemişse 1. Sarmalayıcı: atlas_kare_sayisi3d()."},
