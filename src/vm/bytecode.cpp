...
void chunk_write(Chunk *chunk, uint8_t byte, int line) {
  ...
  if (chunk->code_length >= chunk->code_capacity) {
    ...
    chunk->code =
        static_cast<uint8_t *>(realloc(chunk->code, chunk->code_capacity));
  }
...
int chunk_add_constant(Chunk *chunk, Constant constant) {
  ...
  if (chunk->const_count >= chunk->const_capacity) {
    ...
    chunk->constants = static_cast<Constant *>(
        realloc(chunk->constants, chunk->const_capacity * sizeof(Constant)));
  }
