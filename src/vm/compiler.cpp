...
Compiler *compiler_create() {
  Compiler *compiler = static_cast<Compiler*>(malloc(sizeof(Compiler)));
...
static LocalType infer_expression_type(Compiler *compiler, ASTNode_C *node) {
  if (!node)
    return LOCAL_TYPE_UNKNOWN;
...
static int is_int_binary_op(Compiler *compiler, ASTNode_C *node) {
...
void compile_expression(Compiler *compiler, ASTNode_C *node) {
  if (!node)
    return;
  ...
  case AST_FUNCTION_CALL: {
    ...
    // We should ensure OP_PRINT pushes nullptr/Void or compile_statement handles
...
void compile_statement(Compiler *compiler, ASTNode_C *node) {
  ...
  case AST_FUNCTION_DECL: {
    ...
    CompiledFunction *func =
        static_cast<CompiledFunction*>(malloc(sizeof(CompiledFunction)));
  ...
  case AST_TRY_CATCH: {
    ...
    if (node->catch_var) {
      ...
      // We should emit OP_POP if catch_var is nullptr.
      emit_byte(compiler, OP_POP, node->line);
    }
