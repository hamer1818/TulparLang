...
#endif
    {
      ...
      TARGET(OP_CONST_FUNC) : {
        ...
        if (src->code_length > 0) {
          dest->code = static_cast<uint8_t*>(malloc(src->code_length));
        ...
        if (src->const_count > 0) {
          dest->constants =
              static_cast<Constant*>(malloc(src->const_count * sizeof(Constant)));
          ...
          if (sa->capacity > sa->length) {
            ...
            if (new_len <= sa->capacity) {
              ...
              int new_cap = (sa->capacity * 2) > new_len ? (sa->capacity * 2)
                                                         : (new_len + 16);
              sa->chars =
                  static_cast<char *>(realloc(sa->chars, new_cap + 1));
              sa->capacity = new_cap;
      ...
      TARGET(OP_CALL_BUILTIN) : {
        ...
        case 71: { // replace(str, old, new)
          ...
          if (IS_STRING(strVal) && IS_STRING(oldVal) && IS_STRING(newVal)) {
            ...
            char *new_str = AS_STRING(newVal)->chars;
            ...
            if (old_len == 0) {
              ...
              char *res = static_cast<char *>(malloc(new_size + 1));
              char *rp = res;
              p = s;
              char *found;
        ...
        case 72: { // substring(str, start, len) OR (str, start, end)?
          ...
          if (IS_STRING(strVal)) {
            ...
            char *res = static_cast<char *>(malloc(sliceLen + 1));
        ...
        case 76: { // trim(str) -> removes leading/trailing whitespace
          ...
          if (IS_STRING(strVal)) {
            ...
            char *trimmed = static_cast<char *>(malloc(new_len + 1));
        ...
        case 77: { // contains(haystack, needle) -> bool
          ...
          if (IS_STRING(haystackVal) && IS_STRING(needleVal)) {
            ...
            vm_push(vm, VM_BOOL(strstr(haystack, needle) != nullptr));
          } else {
            vm_push(vm, VM_BOOL(0));
          }
          break;
        ...
        case 78: { // upper(str) -> UPPERCASE string
          ...
          if (IS_STRING(strVal)) {
            ObjString *s = AS_STRING(strVal);
            char *result = static_cast<char *>(malloc(s->length + 1));
          ...
          while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            for (int i = 0; i < col_count; i++) {
              ...
              if (row->capacity < row->count + 1) {
                ...
                row->keys = static_cast<ObjString **>(
                    realloc(row->keys, sizeof(ObjString *) * row->capacity));
                row->values = static_cast<VMValue *>(
          ...
          if (AS_OBJ(targetVal)->type == OBJ_ARRAY) {
            ...
            if (!found) {
              ...
              if (obj->capacity < obj->count + 1) {
                ...
                obj->values = static_cast<VMValue *>(realloc(
      ...
      TARGET(OP_OBJECT_GET) : {
        ...
        if (!found) {
          // Return void/nullptr? Or error?
          // Usually undefined property -> nullptr/void
          result = VM_VOID();
        }
        vm_push(vm, result);
        DISPATCH();
        ...
        if (!found) {
          if (obj->capacity < obj->count + 1) {
            ...
            obj->keys = (ObjString **)realloc(obj->keys, sizeof(ObjString *) *
                                                             obj->capacity);
            obj->values = static_cast<VMValue *>(
                realloc(obj->values, sizeof(VMValue) * obj->capacity));
          }
          obj->keys[obj->count] = key;
          obj->values[obj->count] = val;
          obj->count++;
