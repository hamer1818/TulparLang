...
#ifdef TULPAR_DEBUG
  #define VM_BOUNDS_CHECK(cond, msg) do { if (!(cond)) { fprintf(stderr, "VM Error: %s\n", msg); abort(); } } while(0)
  #define VM_ASSERT(cond) do { if (!(cond)) { fprintf(stderr, "VM Assert failed: %s:%d\n", __FILE__, __LINE__); abort(); } } while(0)
#else
...
typedef struct ObjFunction {
  ...
  struct JITCompiledFunc *jit_code;   // JIT compiled native code (NULL if not compiled)
} ObjFunction;
