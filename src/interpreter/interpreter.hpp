...
typedef struct {
  char *name;
  ASTNode_C *node; // Fonksiyonun AST düğümü
} Function;
...
typedef struct {
  ...
  int field_count;
  struct ASTNode_C **field_defaults; // parser'dan gelen default ifadeler
} TypeDef;
...
typedef struct {
  ...
  ASTNode_C **retained_modules; // import edilen AST'leri sakla
  int retained_count;
