...
// ============================================================================
// VALUE FONKS─░YONLARI - OPT─░M─░ZE ED─░LM─░┼Ş
// ============================================================================
...
// Pool'dan h─▒zl─▒ allocation (malloc'tan 10x+ h─▒zl─▒)
...
// Pool s─▒f─▒rlama (b├╝y├╝k i┼şlemlerden sonra)
...
Value *value_copy(Value *val) {
  ...
  // ========================
  // FAZ 1: H─▒zl─▒ path - cache/singleton de─şerleri do─şrudan d├Ând├╝r
  // ========================

  // Singleton de─şerler - do─şrudan d├Ând├╝r
  if (val == val_true || val == val_false || val == val_void) {
    return val;
  }

  // Integer cache - do─şrudan d├Ând├╝r
  if (val->type == VAL_INT) {
    long long v = val->data.int_val;
    if (v >= INT_CACHE_MIN && v <= INT_CACHE_MAX) {
      return val; // Cache'deki de─şeri d├Ând├╝r
    }
  }

  // Bool ve void i├ğin singleton d├Ând├╝r
  ...
  // Di─şer tipler i├ğin pool'dan allocation
  ...
  case VAL_MUTEX: {
    // Mutex handle kopyala (Shallow copy - ayn─▒ mutex'i i┼şaret eder)
...
void value_free(Value *val) {
  ...
  // ========================
  // FAZ 1: Cache/Singleton/Pool kontrol├╝
  // ========================

  // Singleton de─şerlerini free etme
  if (val == val_true || val == val_false || val == val_void) {
    return;
  }

  // Integer cache'deki de─şerleri free etme
  if (val->type == VAL_INT) {
    long long v = val->data.int_val;
    if (v >= INT_CACHE_MIN && v <= INT_CACHE_MAX) {
  ...
  // Pool'daki de─şerleri free etme
  if (val >= &value_pool[0] && val < &value_pool[VALUE_POOL_SIZE]) {
    // Pool'da, string gibi kaynaklar─▒ temizle ama struct'─▒ free etme
    ...
    return; // Pool'da - struct'─▒ free etme
  }

  // Normal malloc'lanm─▒┼ş de─şerler i├ğin orijinal logic
  if (val->type == VAL_STRING && val->data.string_val) {
    free(val->data.string_val);
  }
...
// Forward declarations
Value *interpreter_eval_expression(Interpreter *interp, ASTNode_C *node);
void interpreter_execute_statement(Interpreter *interp, ASTNode_C *node);
...
Value *interpreter_eval_expression(Interpreter *interp, ASTNode_C *node) {
  ...
  case AST_ARRAY_ACCESS: {
    // Array/Object eri┼şimi: arr[0] or obj["key"] or nested arr[0][1]["key"]
    Value *container = NULL;

    // Zincirleme eri┼şim mi? (left var m─▒?)
    if (node->left) {
      // Nested access: ├Ânce left'i de─şerlendir
    ...
    if (container->type == VAL_ARRAY) {
      ...
      // Left'ten gelen container'─▒ temizle
      if (node->left)
        value_free(container);
      return result;
    }
  ...
  case AST_FUNCTION_CALL: {
    // Built-in fonksiyonlar─▒ kontrol et
    ...
    // toJson(value, pretty=false) - JSON string ├╝ret
    if (strcmp(node->name, "toJson") == 0) {
      if (node->argument_count >= 1) {
        ...
        if (!pretty) {
          ...
          // Pretty-print: basit girintileme (2 bo┼şluk)
          // Not: h─▒zl─▒ ├ğ├Âz├╝m olarak minify ├ğ─▒kt─▒s─▒n─▒ kullan─▒yoruz; geni┼şletme
          // ileri s├╝r├╝mde
          js = value_to_json_string(v);
        }
        value_free(v);
    ...
    if (strcmp(node->name, "fromJson") == 0) {
      if (node->argument_count >= 1) {
        ...
        if (!target)
          return v;
        // target tipe d├Ând├╝r (object beklenir)
        if (v->type != VAL_OBJECT) {
          value_free(v);
          return value_create_void();
        }
        // ─░┼şaretle ve tip kontrol├╝
    ...
    // length() - dizi veya string uzunlu─şu
    ...
    if (strcmp(node->name, "push") == 0) {
      if (node->argument_count >= 2) {
        // ─░lk arg├╝man: dizi de─şi┼şkeni (identifier olarak gelir)
...
// ========================================================================
    ...
    if (strcmp(node->name, "pow") == 0 && node->argument_count >= 2) {
      // E─şer iki arg├╝man da integer ise BigInt ile hesapla
      ...
      if ((a0->type == VAL_INT || a0->type == VAL_BIGINT) &&
        ...
        if (e < 0) {
          // Negatif ├╝s desteklenmiyor -> float
          double xd =
    ...
    // mod(a, b) - tamsay─▒ mod (BigInt destekli)
    ...
    // random() - 0 ile 1 aras─▒ rastgele say─▒
    ...
    // randint(a, b) - a ile b aras─▒ rastgele tam say─▒
...
#define GET_STR_ARG(idx)                                                       \
  ({                                                                           \
    ...
    // trim(s) - ba┼ş ve sondaki bo┼şluklar─▒ sil
    if (strcmp(node->name, "trim") == 0 && node->argument_count >= 1) {
      ...
      if (arg->type == VAL_STRING) {
        ...
        // Ba┼ştan bo┼şluklar─▒ atla
        while (*start && isspace((unsigned char)*start))
          start++;

        // Sondan bo┼şluklar─▒ atla
        while (end > start && isspace((unsigned char)*end))
          end--;

        // Yeni string olu┼ştur
        int len = end - start + 1;
    ...
    if (strcmp(node->name, "replace") == 0 && node->argument_count >= 3) {
      ...
      if (str_val->type == VAL_STRING && old_val->type == VAL_STRING &&
        ...
        char *new_str = new_val->data.string_val;

        int old_len = strlen(old);
        int new_len = strlen(new_str);

        // Ka├ğ kez ge├ğiyor say
        int count = 0;
        char *p = str;
    ...
    // contains(s, sub) - alt string var m─▒
    ...
    // startsWith(s, prefix) - ile ba┼şl─▒yor mu
    ...
    // indexOf(s, sub) - ilk konum (-1 = bulunamad─▒)
    ...
    // split(s, delimiter) - string'i b├Âl ve dizi d├Ând├╝r
    if (strcmp(node->name, "split") == 0 && node->argument_count >= 2) {
      ...
      if (str_val->type == VAL_STRING && delim_val->type == VAL_STRING) {
        ...
        // ├ûnce ka├ğ par├ğa olaca─ş─▒n─▒ say
        int count = 1;
        char *temp = str;
    ...
    // join(separator, array) - diziyi birle┼ştir
    ...
    // Kullan─▒c─▒ tan─▒ml─▒ fonksiyonlar
    // Metot ├ğa─şr─▒s─▒ m─▒? (receiver varsa)
    if (node->receiver) {
    ...
    if (!func) {
      ...
      if (t) {
        // Arg├╝manlar─▒ de─şerlendir ve object olu┼ştur (named args destekli)
        Value *obj = value_create_object();
        // Type i┼şareti
    ...
    // Parametreleri ├Ânce de─şerlendir (mevcut scope'ta)
    Value **arg_values =
        (Value **)malloc(sizeof(Value *) * node->argument_count);
    for (int i = 0; i < node->argument_count; i++) {
      arg_values[i] = interpreter_eval_expression(interp, node->arguments[i]);
...
// ============================================================================
// STATEMENT ├çALI┼ŞTIRMA (Statement Execution)
// ============================================================================
...
void interpreter_execute_statement(Interpreter *interp, ASTNode_C *node) {
  ...
  case AST_TYPE_DECL: {
    ...
    t->field_defaults = (ASTNode_C **)malloc(sizeof(ASTNode_C *) * t->field_count);
  ...
  case AST_VARIABLE_DECL: {
    ...
    if (node->right) {
      ...
      // E─şer tipli array tan─▒m─▒ ise ve de─şer de array ise, tip kontrol├╝ yap
  ...
  case AST_ASSIGNMENT: {
    ...
    // E─şer sol taraf array/object eri┼şimi zinciri ise
    if (node->left && node->left->type == AST_ARRAY_ACCESS) {
      // Zinciri ├ğ├Âz: base name ve segment listesi
      ASTNode_C *seg = node->left;
      // En sola kadar git
      while (seg->left)
        seg = seg->left;
      if (!seg->name) {
      ...
      ASTNode_C **nodes = (ASTNode_C **)malloc(sizeof(ASTNode_C *) * depth);
      tmp = node->left;
      for (int i = depth - 1; i >= 0; i--) {
        nodes[i] = tmp;
        tmp = tmp->left;
      }
      // nodes[0] en sol (base), nodes[depth-1] en sa─ş (target)
      for (int i = 0; i < depth - 1; i++) {
        ASTNode_C *n = nodes[i];
        // index'i de─şerlendir
  ...
  case AST_IMPORT: {
    ...
    ASTNode_C *imported_ast = parser_parse(parser);
    ...
    if (interp->retained_count >= interp->retained_capacity) {
      ...
      interp->retained_modules = (ASTNode_C **)realloc(
          interp->retained_modules, sizeof(ASTNode_C *) * new_capacity);
  ...
  case AST_FOR: {
    // For d├Âng├╝s├╝ i├ğin yeni scope olu┼ştur (loop variable i├ğin)
    SymbolTable *old_scope = interp->current_scope;
    interp->current_scope = symbol_table_create(old_scope);

    // Init statement'─▒ ├ğal─▒┼şt─▒r (int i = 0)
    if (node->init) {
    ...
    while (1) {
      ...
      if (interp->should_continue) {
        interp->should_continue = 0;
        // Continue: increment'i ├ğal─▒┼şt─▒r ve devam et
        if (node->increment) {
      ...
      // Increment statement'─▒ ├ğal─▒┼şt─▒r (i = i + 1)
      if (node->increment) {
        interpreter_execute_statement(interp, node->increment);
      }
    }
  ...
  case AST_FOR_IN: {
    ...
    if (iterable_val->type == VAL_INT) {
      ...
      // 0'dan count'a kadar d├Âng├╝
      for (int i = 0; i < count; i++) {
        if (interp->should_return || interp->should_break)
          break;

        // Iterator de─şi┼şkenini g├╝ncelle
...
// ============================================================================
// ANA ─░NTERPRETER FONKS─░YONU
// ============================================================================

void interpreter_execute(Interpreter *interp, ASTNode_C *node) {
  interpreter_execute_statement(interp, node);
}

Value *interpreter_eval(Interpreter *interp, ASTNode_C *node) {
...
// Helper: Interpreter clone (Thread i├ğin)
// Sadece fonksiyonlar─▒ ve o anki global de─şi┼şkenlerin snapshot'─▒n─▒ al─▒r
static Interpreter *interpreter_clone(Interpreter *src) {
  Interpreter *dest = interpreter_create();

  // Fonksiyonlar─▒ kopyala (kod de─şi┼şmez, referans)
  ...
  // Global de─şi┼şkenleri kopyala (Deep Copy gerekli!)
