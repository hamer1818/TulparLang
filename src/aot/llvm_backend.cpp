...
// TEMEL BLOK YARATMA — HER ZAMAN MODULUN BAGLAMINDA.
//
// LLVM-C'nin `LLVMAppendBasicBlock`i blogu KURESEL baglamda yaratiyor
// (basligindaki tanim aynen bu: `...InContext(LLVMGetGlobalContext(), ...)`).
// Bizim modulumuz ise `LLVMContextCreate()` ile ACILAN AYRI bir baglamda.
// Ikisi karisinca ayni yazilan tur iki AYRI Type nesnesi oluyor ve olculen
// sonuc su (2026-09-06):
//
//   * `LLVMVerifyModule` optimizasyondan SONRA "MDNode context does not
//     match Module context" veriyordu — her derlemede butun modulun
//     yazdir/yeniden-ayristir turuna girmesi bu yuzdendi.
//   * Dongu vektorlestirici, ureteci BIZIM blogumuzda konumlandiginda
//     sabitlerini o blogun baglamindan aliyor: `add <2 x i64> %vec.ind,
//     <2 x i64> splat (i64 2)` — iki islenen ayni yazilip FARKLI Type
//     oluyor, dogrulayici "Both operands ... not of the same type" diyor
//     ve boru hatti O3'ten O1'e dusuyordu. Yani vektorlesen her dongu
//     sessizce optimizasyonsuz kaliyordu.
//
// Yeni blok yaratan her yer buradan gecmeli.
static LLVMBasicBlockRef append_bb(LLVMBackend *backend, LLVMValueRef fn,
                                   const char *name) {
  return LLVMAppendBasicBlockInContext(backend->context, fn, name);
}
...
static LLVMValueRef build_checked_div(LLVMBackend *backend, LLVMValueRef l,
  ...
  LLVMBasicBlockRef bb_bad = append_bb(backend, fn, "div.bad");
  LLVMBasicBlockRef bb_ok = append_bb(backend, fn, "div.ok");
  LLVMBasicBlockRef bb_done = append_bb(backend, fn, "div.done");
...
static void collect_declared_locals(ASTNode_C *node, std::unordered_set<std::string> &declared) {
  ...
  if (node->type != AST_FUNCTION_DECL && node->type != AST_LAMBDA) {
    collect_declared_locals(node->left, declared);
    collect_declared_locals(node->right, declared);
    collect_declared_locals(node->body, declared);
    collect_declared_locals(node->condition, declared);
    collect_declared_locals(node->then_branch, declared);
    collect_declared_locals(node->else_branch, declared);
    collect_declared_locals(node->init, declared);
    collect_declared_locals(node->increment, declared);
    collect_declared_locals(node->iterable, declared);
    collect_declared_locals(node->return_value, declared);
    collect_declared_locals(node->index, declared);
    collect_declared_locals(node->receiver, declared);
    collect_declared_locals(node->callee, declared);
    collect_declared_locals(node->try_block, declared);
    collect_declared_locals(node->catch_block, declared);
    collect_declared_locals(node->finally_block, declared);
    collect_declared_locals(node->throw_expr, declared);
    for (int i = 0; i < node->statement_count; i++) collect_declared_locals(node->statements[i], declared);
    for (int i = 0; i < node->argument_count; i++) collect_declared_locals(node->arguments[i], declared);
    for (int i = 0; i < node->element_count; i++) collect_declared_locals(node->elements[i], declared);
    for (int i = 0; i < node->object_count; i++) collect_declared_locals(node->object_values[i], declared);
  }
}

static void collect_function_declared_vars(ASTNode_C *func, std::unordered_set<std::string> &declared) {
  for (int i = 0; i < func->param_count; i++) {
    if (func->parameters[i] && func->parameters[i]->name) {
      declared.insert(func->parameters[i]->name);
    }
  }
  collect_declared_locals(func->body, declared);
}

static void collect_free_variables(ASTNode_C *node, 
                                   std::unordered_set<std::string> declared_inner, 
                                   std::unordered_set<std::string> &free_vars) {
  if (!node) return;
  if (node->type == AST_VARIABLE_DECL) {
    if (node->name) {
      declared_inner.insert(node->name);
    }
  }
  if (node->type == AST_IDENTIFIER) {
    if (node->name && declared_inner.find(node->name) == declared_inner.end()) {
  ...
  if (node->type == AST_FUNCTION_DECL || node->type == AST_LAMBDA) {
    for (int i = 0; i < node->param_count; i++) {
      if (node->parameters[i] && node->parameters[i]->name) {
        declared_inner.insert(node->parameters[i]->name);
      }
    }
    collect_free_variables(node->body, declared_inner, free_vars);
  } else {
    collect_free_variables(node->left, declared_inner, free_vars);
    collect_free_variables(node->right, declared_inner, free_vars);
    collect_free_variables(node->body, declared_inner, free_vars);
    collect_free_variables(node->condition, declared_inner, free_vars);
    collect_free_variables(node->then_branch, declared_inner, free_vars);
    collect_free_variables(node->else_branch, declared_inner, free_vars);
    collect_free_variables(node->init, declared_inner, free_vars);
    collect_free_variables(node->increment, declared_inner, free_vars);
    collect_free_variables(node->iterable, declared_inner, free_vars);
    collect_free_variables(node->return_value, declared_inner, free_vars);
    collect_free_variables(node->index, declared_inner, free_vars);
    collect_free_variables(node->receiver, declared_inner, free_vars);
    collect_free_variables(node->callee, declared_inner, free_vars);
    collect_free_variables(node->try_block, declared_inner, free_vars);
    collect_free_variables(node->catch_block, declared_inner, free_vars);
    collect_free_variables(node->finally_block, declared_inner, free_vars);
    collect_free_variables(node->throw_expr, declared_inner, free_vars);
    for (int i = 0; i < node->statement_count; i++) collect_free_variables(node->statements[i], declared_inner, free_vars);
    for (int i = 0; i < node->argument_count; i++) collect_free_variables(node->arguments[i], declared_inner, free_vars);
    for (int i = 0; i < node->element_count; i++) collect_free_variables(node->elements[i], declared_inner, free_vars);
    for (int i = 0; i < node->object_count; i++) collect_free_variables(node->object_values[i], declared_inner, free_vars);
  }
}
...
struct NestedVisitor {
      static void visit(ASTNode_C *curr, const std::unordered_set<std::string> &outer_declared, std::unordered_set<std::string> &captured_vars) {
        ...
        if (curr->type == AST_FUNCTION_DECL || curr->type == AST_LAMBDA) {
          std::unordered_set<std::string> inner_declared;
          for (int i = 0; i < curr->param_count; i++) {
            if (curr->parameters[i] && curr->parameters[i]->name) {
              inner_declared.insert(curr->parameters[i]->name);
            }
          }
          std::unordered_set<std::string> free_vars;
          collect_free_variables(curr->body, inner_declared, free_vars);
          for (const auto &v : free_vars) {
            if (outer_declared.find(v) != outer_declared.end()) {
              captured_vars.insert(v);
            }
          }
        }
        visit(curr->left, outer_declared, captured_vars);
        visit(curr->right, outer_declared, captured_vars);
        visit(curr->body, outer_declared, captured_vars);
        visit(curr->condition, outer_declared, captured_vars);
        visit(curr->then_branch, outer_declared, captured_vars);
        visit(curr->else_branch, outer_declared, captured_vars);
        visit(curr->init, outer_declared, captured_vars);
        visit(curr->increment, outer_declared, captured_vars);
        visit(curr->iterable, outer_declared, captured_vars);
        visit(curr->return_value, outer_declared, captured_vars);
        visit(curr->index, outer_declared, captured_vars);
        visit(curr->receiver, outer_declared, captured_vars);
        visit(curr->callee, outer_declared, captured_vars);
        visit(curr->try_block, outer_declared, captured_vars);
        visit(curr->catch_block, outer_declared, captured_vars);
        visit(curr->finally_block, outer_declared, captured_vars);
        visit(curr->throw_expr, outer_declared, captured_vars);
        for (int i = 0; i < curr->statement_count; i++) visit(curr->statements[i], outer_declared, captured_vars);
        for (int i = 0; i < curr->argument_count; i++) visit(curr->arguments[i], outer_declared, captured_vars);
...
static void analyze_module_captures(LLVMBackend *backend, ASTNode_C *node, int depth, ASTNode_C *current_func) {
  ...
  if (node->type == AST_FUNCTION_DECL || node->type == AST_LAMBDA) {
    ...
    for (int i = 0; i < node->param_count; i++) {
      if (node->parameters[i] && node->parameters[i]->name) {
        std::string name = node->parameters[i]->name;
        if (captured.find(name) != captured.end()) {
          cd->slots[node][name] = slot++;
        }
      }
    }
  ...
  analyze_module_captures(backend, node->left, depth, current_func);
  analyze_module_captures(backend, node->right, depth, current_func);
  analyze_module_captures(backend, node->body, depth, current_func);
  analyze_module_captures(backend, node->condition, depth, current_func);
  analyze_module_captures(backend, node->then_branch, depth, current_func);
  analyze_module_captures(backend, node->else_branch, depth, current_func);
  analyze_module_captures(backend, node->init, depth, current_func);
  analyze_module_captures(backend, node->increment, depth, current_func);
  analyze_module_captures(backend, node->iterable, depth, current_func);
  analyze_module_captures(backend, node->return_value, depth, current_func);
  analyze_module_captures(backend, node->index, depth, current_func);
  analyze_module_captures(backend, node->receiver, depth, current_func);
  analyze_module_captures(backend, node->callee, depth, current_func);
  analyze_module_captures(backend, node->try_block, depth, current_func);
  analyze_module_captures(backend, node->catch_block, depth, current_func);
  analyze_module_captures(backend, node->finally_block, depth, current_func);
  analyze_module_captures(backend, node->throw_expr, depth, current_func);
  for (int i = 0; i < node->statement_count; i++) analyze_module_captures(backend, node->statements[i], depth, current_func);
  for (int i = 0; i < node->argument_count; i++) analyze_module_captures(backend, node->arguments[i], depth, current_func);
  for (int i = 0; i < node->element_count; i++) analyze_module_captures(backend, node->elements[i], depth, current_func);
  for (int i = 0; i < node->object_count; i++) analyze_module_captures(backend, node->object_values[i], depth, current_func);
}
...
static const TameBuiltin k_tame_builtins[] = {
    ...
    {"tm_view_left", "aot_tm_view_left_ptr", 0},
    {"tm_view_right", "aot_tm_view_right_ptr", 0},
    {"tm_view_top", "aot_tm_view_top_ptr", 0},
    {"tm_view_bottom", "aot_tm_view_bottom_ptr", 0},
    {"tm_accel_x", "aot_tm_accel_x_ptr", 0},
    {"tm_accel_y", "aot_tm_accel_y_ptr", 0},
    {"tm_accel_z", "aot_tm_accel_z_ptr", 0},
    ...
    {"tm3_shadows_active", "aot_tm3_shadows_active_ptr", 0},
    // 3D (Faz 5) — doku / materyal / gökyüzü
    {"tm3_texture", "aot_tm3_texture_ptr", 3},
    {"tm3_material", "aot_tm3_material_ptr", 2},
...
void declare_runtime_functions(LLVMBackend *backend) {
  ...
  backend->func_aot_create_closure = LLVMAddFunction(backend->module, "aot_create_closure", create_cls_type);

  // aot_call_closure(ptr, ptr, int32) -> VMValue
...
void enter_scope(LLVMBackend *backend) {
  Scope *scope = static_cast<Scope*>(calloc(1, sizeof(Scope)));
  scope->count = 0;
...
LLVMValueRef find_env_for_decl(LLVMBackend *backend, ASTNode_C *decl_node) {
  ...
  if (curr == decl_node) {
    return backend->current_env_ptr;
  }
  
  LLVMValueRef env = nullptr;
  if (curr && curr->type == AST_LAMBDA) {
    env = LLVMGetParam(backend->current_function, 1);
  } else {
    env = backend->current_env_ptr;
  }
  
  FuncStackNode *stack_node = backend->func_stack;
  if (stack_node) {
    stack_node = stack_node->parent;
  }
  
  while (stack_node) {
    ASTNode_C *func_node = stack_node->node;
    if (func_node == decl_node) {
      break;
    }
    if (func_node && cd && cd->slots.find(func_node) != cd->slots.end() && !cd->slots[func_node].empty()) {
      LLVMValueRef idx_val = LLVMConstInt(backend->int32_type, 0, 0);
      LLVMValueRef get_args[] = {env, idx_val};
      LLVMValueRef parent_vmval = llvm_call_vmvalue_func(
          backend, backend->func_vm_array_get, get_args, 2, "parent_env_vmval");
      env = llvm_extract_vm_val_ptr(backend, parent_vmval);
    }
    stack_node = stack_node->parent;
  }
  return env;
...
//
// Liste ELLE dogrulandi ve kasten kisa: her biri ya salt okuma (`len`) ya da
// saf sayisal/donusum. Buraya bir ad eklemeden once sorulacak soru "saf mi"
// DEGIL, "bir dizinin count/items_/idata alanini degistirebilir mi" — ve
// dolayli olarak da: kullanici koduna geri donebilir mi (`call`, `map`,
// karsilastiricili `sort` gibi) — donebiliyorsa girmez.
//
// Ayirma yapan yerlesikler (`toStr` vb.) sorun degil: arena obek-zincirli
// bump ayirici, bloklar asla tasinmiyor, yani onbellekteki isaretci gecerli
// kaliyor. Yine de listeyi dar tutuyoruz.
static int shape_pure_call(const char *name, void *ctx) {
  if (!name) return 0;
  LLVMBackend *backend = (LLVMBackend *)ctx;
  // Kullanici ayni adi tanimladiysa yerlesik degil: govdesi her seyi yapabilir.
  for (int i = 0; i < backend->function_count; i++)
    if (backend->functions[i].name && strcmp(backend->functions[i].name, name) == 0)
      return 0;
  static const char *k_pure[] = {"len",  "length", "abs",     "min",
                                 "max",  "sqrt",   "pow",     "floor",
                                 "ceil", "round",  "toInt",   "toFloat",
                                 "ord",  "chr"};
  for (const char *p : k_pure)
    if (strcmp(p, name) == 0) return 1;
  return 0;
}

// Dongu BASINDA bir buyuklugu i64 olarak oku; kutuluysa "gercekten int mi"
// kosulunu `*ok`a ekle.
//
// `while` surumlemesinin sinavi (v>=0, STEP>0, UB<count) ham i64
// karsilastirmasi. Ad native bir yuvada duruyorsa (en ust kapsamdaki
// `int n = ...` boyle) dogrudan okunuyor, ek kosul gerekmiyor. Kutulu bir
// yuvadaysa (`while` govdesindeki `int k = ...` boyle — SROA sonrasi i64
// gorunse de yuva VMValue) etiket denetimi ekleniyor. Denetim DONGU
// DISINDA, bir kez: sicak yola hicbir sey binmiyor.
//
// ⚠ Bu fonksiyon KOD URETIYOR. Cagrildiktan sonra surumlemeden
// vazgecilirse geriye olu yukler kalir — gecerli IR, LLVM temizliyor.
// Ama TEMEL BLOK YARATILDIKTAN SONRA cagrilmamali (bkz. Tuzaklar 6q).
// Surumleme dalina AGIRLIK ver: hizli surum sicak, genel surum SOGUK.
//
// Neden gerekli: iki govde uretmek dis dongunun ICINE ikinci bir kopya
// koyuyor. Elek'te ic dongu dis dongunun govdesinde; agirliksiz halde
// LLVM iki kopyayi yan yana yerlestiriyor ve dis dongunun ayak izi
// buyudugu icin KAZANC KAYBA DONUYORDU (olculdu 2026-09-06: ic dongu tek
// basina 8,69 -> 8,06 ms iyilesirken elek 9,72 -> 10,37 GERILEDI).
// Agirlik, genel surumu sicak yoldan uzaga tasiyor.
static void set_branch_weights(LLVMBackend *backend, LLVMValueRef br,
                               unsigned wt, unsigned wf) {
  if (!br) return;
  LLVMMetadataRef ops[3] = {
      LLVMMDStringInContext2(backend->context, "branch_weights", 14),
      LLVMValueAsMetadata(LLVMConstInt(backend->int32_type, wt, 0)),
      LLVMValueAsMetadata(LLVMConstInt(backend->int32_type, wf, 0))};
  LLVMMetadataRef md = LLVMMDNodeInContext2(backend->context, ops, 3);
  unsigned kind = LLVMGetMDKindIDInContext(backend->context, "prof", 4);
  LLVMSetMetadata(br, kind, LLVMMetadataAsValue(backend->context, md));
}

static LLVMValueRef load_loop_int(LLVMBackend *backend, const char *name,
                                  LLVMValueRef *ok) {
  if (!name) return nullptr;
  if (get_local_type(backend, name) == INFERRED_INT) {
    LLVMValueRef nat = get_local_native(backend, name);
    if (nat)
      return LLVMBuildLoad2(backend->builder, backend->int_type, nat, name);
  }
  ...
  LLVMValueRef tag = LLVMBuildExtractValue(backend->builder, v, 0, "wv.tag");
  LLVMValueRef isint =
      LLVMBuildICmp(backend->builder, LLVMIntEQ, tag,
                    LLVMConstInt(backend->int32_type, 0, 0), "wv.isint");
  *ok = *ok ? LLVMBuildAnd(backend->builder, *ok, isint, "wv.tand") : isint;
...
static void emit_shape_fill(LLVMBackend *backend, const char *name,
  ...
  LLVMValueRef fn = LLVMGetBasicBlockParent(LLVMGetInsertBlock(backend->builder));
  LLVMBasicBlockRef b_ty = append_bb(backend, fn, "shape.ty");
  LLVMBasicBlockRef b_ld = append_bb(backend, fn, "shape.ld");
  LLVMBasicBlockRef b_no = append_bb(backend, fn, "shape.no");
  LLVMBasicBlockRef b_done = append_bb(backend, fn, "shape.done");
...
static LLVMValueRef get_shape_refill_fn(LLVMBackend *backend, int eager) {
  ...
  LLVMBasicBlockRef b_ret = append_bb(backend, fn, "ret");
...
static int emit_shape_cache_for_loop(LLVMBackend *backend, ASTNode_C *cond,
  ...
  if (!tulpar_loop_shape_stable(cond, body, incr, shape_pure_call, backend))
    return saved;
  const char *names[4];
...
LLVMValueRef codegen_expression(LLVMBackend *backend, ASTNode_C *node) {
  ...
  case AST_STRING_LITERAL: {
    ...
    LLVMBasicBlockRef init_bb = append_bb(backend, fn, "strlit.init");
    LLVMBasicBlockRef done_bb = append_bb(backend, fn, "strlit.done");
  ...
  case AST_ARRAY_ACCESS: {
    ...
    if (node->name) {
      ...
      if (val_ptr) {
        left_val = LLVMBuildLoad2(backend->builder, backend->vm_value_type,
                                  val_ptr, node->name);
        // Degisken yuvasi (alloca/kuresel) — asla dizi eleman deposu degil.
        llvm_tbaa_tag(backend, left_val, 0);
      } else {
        char msg[256];
...

    {
      ...
      LLVMBasicBlockRef bb_chk  = append_bb(backend, fn, "arr.chk");
      LLVMBasicBlockRef bb_fast = append_bb(backend, fn, "arr.fast");
      LLVMBasicBlockRef bb_slow = append_bb(backend, fn, "arr.slow");
      LLVMBasicBlockRef bb_done = append_bb(backend, fn, "arr.done");
      LLVMTypeRef i32t = backend->int32_type;
      ...
      if (shp) {
        LLVMBasicBlockRef bb_cached = append_bb(backend, fn, "arr.cached");
        LLVMBasicBlockRef bb_gen = append_bb(backend, fn, "arr.generic");
      ...
      // Hizli yol ikiye ayriliyor: kutulanmamis (idata != NULL) diziden
      // dogrudan i64 yukleniyor — eleman basina 16 degil 8 bayt dokunuluyor.
      LLVMBasicBlockRef bb_ubox  = append_bb(backend, fn, "arr.unboxed");
      LLVMBasicBlockRef bb_boxed = append_bb(backend, fn, "arr.boxed");
      LLVMPositionBuilderAtEnd(backend->builder, bb_fast);
      LLVMValueRef id_ptr = LLVMBuildStructGEP2(
          backend->builder, backend->obj_array_type, objp, 5, "arr.idata.ptr");
      LLVMValueRef idata = LLVMBuildLoad2(backend->builder, backend->ptr_type,
                                          id_ptr, "arr.idata");
      llvm_tbaa_tag(backend, idata, 0);
      LLVMBuildCondBr(backend->builder,
                      LLVMBuildIsNotNull(backend->builder, idata, "arr.isubox"),
                      bb_ubox, bb_boxed);
      ...
      LLVMValueRef ubox_val = llvm_vm_val_int_val(backend, iraw);
      LLVMBasicBlockRef ubox_end = LLVMGetInsertBlock(backend->builder);
      LLVMBuildBr(backend->builder, bb_done);

      LLVMPositionBuilderAtEnd(backend->builder, bb_boxed);
      ...
      llvm_tbaa_tag(backend, fast_val, 1);
      LLVMBasicBlockRef boxed_end = LLVMGetInsertBlock(backend->builder);
      LLVMBuildBr(backend->builder, bb_done);
  ...
  case AST_IDENTIFIER: {
    ...
    if (val_ptr) {
      ...
      LLVMValueRef ident_val = LLVMBuildLoad2(
          backend->builder, backend->vm_value_type, val_ptr, node->name);
      llvm_tbaa_tag(backend, ident_val, 0);
      return ident_val;
    }
  ...
  case AST_UNARY_OP: {
    ...
    if (node->op == TOKEN_MINUS) {
      ...
      LLVMBasicBlockRef check_float =
          append_bb(backend, func, "unary_check_float");
      LLVMBasicBlockRef int_block = append_bb(backend, func, "unary_int");
      LLVMBasicBlockRef float_block = append_bb(backend, func, "unary_float");
      LLVMBasicBlockRef fallback_block =
          append_bb(backend, func, "unary_fallback");
      LLVMBasicBlockRef merge_block = append_bb(backend, func, "unary_merge");
  ...
  case AST_FUNCTION_CALL: {
    ...
    if (node->name && strcmp(bi_name, "len") == 0 &&
      ...
      // `for (int i = 0; i < len(a); ...)` Tulpar'in en yaygin dongu kalibi
      // ve boyle her yinelemede bir runtime cagrisi yapiyordu — olculdu:
      // ayni dongu len()'siz 5,1 ms, len()'li 12,7 ms.
      //
      // len_slot dizi olmayan bir deger icin -1 tutuyor (dizgi, json, ...);
      // o zaman eski cagriya dusuyoruz, yani anlam birebir korunuyor.
      // Dal dongu-degismezi oldugu icin LLVM onu disari tasiyor.
      ASTNode_C *larg = node->arguments[0];
      LLVMBackend::ArrShapeEntry *lshp =
      ...
      if (lshp) {
        LLVMValueRef fnl =
            LLVMGetBasicBlockParent(LLVMGetInsertBlock(backend->builder));
        LLVMBasicBlockRef lb_hit = append_bb(backend, fnl, "len.cached");
        LLVMBasicBlockRef lb_call = append_bb(backend, fnl, "len.call");
        LLVMBasicBlockRef lb_done = append_bb(backend, fnl, "len.done");
        LLVMValueRef cl = LLVMBuildLoad2(backend->builder, backend->int_type,
                                         lshp->len_slot, "len.cl");
        LLVMBuildCondBr(
            backend->builder,
            LLVMBuildICmp(backend->builder, LLVMIntSGE, cl,
                          LLVMConstInt(backend->int_type, 0, 0), "len.ok"),
            lb_hit, lb_call);

        LLVMPositionBuilderAtEnd(backend->builder, lb_hit);
        LLVMValueRef hit_val = llvm_vm_val_int_val(backend, cl);
        LLVMBasicBlockRef hit_end = LLVMGetInsertBlock(backend->builder);
        LLVMBuildBr(backend->builder, lb_done);

        LLVMPositionBuilderAtEnd(backend->builder, lb_call);
        LLVMValueRef carg = codegen_expression(backend, larg);
        LLVMValueRef cap = llvm_build_alloca_at_entry(
            backend, backend->vm_value_type, "len_arg");
        LLVMBuildStore(backend->builder, carg, cap);
        LLVMValueRef cargs[] = {LLVMBuildBitCast(backend->builder, cap,
                                                 backend->ptr_type, "len_ap")};
        LLVMValueRef cres = LLVMBuildCall2(
            backend->builder, LLVMGlobalGetValueType(backend->func_aot_len),
            backend->func_aot_len, cargs, 1, "len_result");
        LLVMValueRef call_val = llvm_vm_val_int_val(backend, cres);
        LLVMBasicBlockRef call_end = LLVMGetInsertBlock(backend->builder);
        LLVMBuildBr(backend->builder, lb_done);

        LLVMPositionBuilderAtEnd(backend->builder, lb_done);
        LLVMValueRef lphi = LLVMBuildPhi(backend->builder,
                                         backend->vm_value_type, "len.res");
        LLVMValueRef linc[] = {hit_val, call_val};
        LLVMBasicBlockRef lbb[] = {hit_end, call_end};
        LLVMAddIncoming(lphi, linc, lbb, 2);
        return lphi;
      }
      LLVMValueRef arg = codegen_expression(backend, node->arguments[0]);
      LLVMValueRef arg_ptr =
...
#undef TYPE_CHECK_FUNC
    ...
    if (strcmp(bi_name, "sb_append") == 0 && node->argument_count >= 2) {
      ...
      LLVMValueRef fnp =
          LLVMGetBasicBlockParent(LLVMGetInsertBlock(backend->builder));
      LLVMBasicBlockRef ab_fast = append_bb(backend, fnp, "sbap.int");
      LLVMBasicBlockRef ab_slow = append_bb(backend, fnp, "sbap.gen");
      LLVMBasicBlockRef ab_done = append_bb(backend, fnp, "sbap.done");
      LLVMValueRef vtag =
    ...
    // Indirect/Closure function call
    bool is_closure_call = false;
    LLVMValueRef closure_val = nullptr;

    if (node->callee) {
      is_closure_call = true;
      closure_val = codegen_expression(backend, node->callee);
    } else if (node->name && get_local_var(backend, node->name) != nullptr) {
      is_closure_call = true;
      ASTNode_C dummy;
  ...
  case AST_LAMBDA: {
    // 1. Generate unique name: "__lambda_N"
    char lam_name[64];
    snprintf(lam_name, sizeof(lam_name), "__lambda_%d", backend->lambda_count++);

    // 2. Define function type: void(VMValue* result_ptr, void* env_ptr, VMValue* arg1, ...)
    int user_param_count = node->param_count;
    int total_params = user_param_count + 2;
    LLVMTypeRef *param_types = (LLVMTypeRef*)malloc(sizeof(LLVMTypeRef) * total_params);
    for (int i = 0; i < total_params; i++) {
      param_types[i] = backend->ptr_type;
    }
    LLVMTypeRef func_type = LLVMFunctionType(backend->void_type, param_types, total_params, 0);
    free(param_types);

    // 3. Create the function
    LLVMValueRef func = LLVMAddFunction(backend->module, lam_name, func_type);

    // 4. Save builder state
    LLVMValueRef prev_function = backend->current_function;
    ...
    backend->current_parent_env = LLVMGetParam(func, 1);

    // 6. Create entry basic block
    ...
    // 8. Allocate environment if it has captured variables
    CaptureData *cd = (CaptureData*)backend->capture_data;
    bool has_captures = (cd->slots.find(node) != cd->slots.end() && !cd->slots[node].empty());
    if (has_captures) {
      int env_size = cd->slots[node].size() + 1;
      LLVMValueRef alloc_args[] = {LLVMConstNull(backend->ptr_type)};
      LLVMValueRef env_obj = LLVMBuildCall2(
          backend->builder,
          LLVMGlobalGetValueType(backend->func_vm_allocate_array),
          backend->func_vm_allocate_array, alloc_args, 1, "lam_alloc_env");
      backend->current_env_ptr = env_obj;

      // Slot 0: Push parent env
      LLVMValueRef wrapped_parent = llvm_build_vm_val_obj(backend, backend->current_parent_env);
      LLVMValueRef val_ptr = LLVMBuildAlloca(backend->builder, backend->vm_value_type, "parent_env_val_ptr");
      LLVMBuildStore(backend->builder, wrapped_parent, val_ptr);
      LLVMValueRef val_void = LLVMBuildBitCast(backend->builder, val_ptr, backend->ptr_type, "parent_env_val_void");
      LLVMValueRef push_args[] = {LLVMConstNull(backend->ptr_type), env_obj, val_void};
      LLVMBuildCall2(backend->builder,
                     LLVMGlobalGetValueType(backend->func_vm_array_push),
                     backend->func_vm_array_push, push_args, 3, "");

      for (int i = 1; i < env_size; i++) {
        LLVMValueRef def_val = llvm_vm_val_int(backend, 0);
        LLVMValueRef slot_ptr = LLVMBuildAlloca(backend->builder, backend->vm_value_type, "env_def_val_ptr");
        LLVMBuildStore(backend->builder, def_val, slot_ptr);
        LLVMValueRef slot_void = LLVMBuildBitCast(backend->builder, slot_ptr, backend->ptr_type, "env_def_val_void");
        LLVMValueRef push_args2[] = {LLVMConstNull(backend->ptr_type), env_obj, slot_void};
        LLVMBuildCall2(backend->builder,
                       LLVMGlobalGetValueType(backend->func_vm_array_push),
                       backend->func_vm_array_push, push_args2, 3, "");
      }
    } else {
    ...
    for (int i = 0; i < user_param_count; i++) {
      if (node->parameters[i]) {
        ...
        LLVMValueRef arg_ptr = LLVMGetParam(func, i + 2);
    ...
    if (node->body->type == AST_BLOCK) {
      ...
      LLVMValueRef val = codegen_expression(backend, node->body);
      LLVMValueRef res_ptr = LLVMGetParam(func, 0);
      LLVMBuildStore(backend->builder, val, res_ptr);
      LLVMBuildRetVoid(backend->builder);
    }
    ...
    // 11. Restore compiler state and pop scope
    exit_scope(backend);
    ...
    LLVMValueRef void_func_ptr = LLVMBuildBitCast(backend->builder, func, backend->ptr_type, "lam_func_ptr");
    LLVMValueRef env_val = prev_env_ptr;
    if (!env_val) {
      if (prev_func_node && prev_func_node->type == AST_LAMBDA) {
        env_val = LLVMGetParam(prev_function, 1);
      } else {
        env_val = LLVMConstNull(backend->ptr_type);
      }
    }
    LLVMValueRef arity_val = LLVMConstInt(backend->int32_type, user_param_count, 0);
    LLVMValueRef create_args[] = {void_func_ptr, env_val, arity_val};
    LLVMValueRef closure_ptr = LLVMBuildCall2(
        backend->builder,
        LLVMGlobalGetValueType(backend->func_aot_create_closure),
        backend->func_aot_create_closure, create_args, 3, "create_cls");
  ...
  case AST_MATCH: {
    ...
    LLVMValueRef func = backend->current_function;
    LLVMBasicBlockRef merge_bb = append_bb(backend, func, "match_end");
    ...
    auto guard = [&](LLVMValueRef cond, LLVMBasicBlockRef fail_bb,
                     const char *nm) {
      LLVMBasicBlockRef cont = append_bb(backend, func, nm);
    ...
    for (int i = 0; i < node->element_count; i++) {
      ...
      LLVMBasicBlockRef next_bb = append_bb(backend, func, "match_next");
  ...
  case AST_TERNARY: {
    ...
    LLVMValueRef func = backend->current_function;
    LLVMBasicBlockRef then_bb = append_bb(backend, func, "tern_then");
    LLVMBasicBlockRef else_bb = append_bb(backend, func, "tern_else");
    LLVMBasicBlockRef merge_bb = append_bb(backend, func, "tern_end");
...
LLVMValueRef codegen_statement(LLVMBackend *backend, ASTNode_C *node) {
  ...
  case AST_ASSIGNMENT: {
    ...
    if (node->left && node->left->type == AST_ARRAY_ACCESS) {
      ...
      // KANITLI ELEMAN YAZMASI — okuma tarafindaki kanitli erisimin ikizi.
      // `for (i = C; i < len(a); i += K)` icinde `a[i] = <ifade>`: 0 <= i <
      // len == count (dizi kutusuz; dongu basinda sinandi ve SURUMLENDI),
      // ve dongudeki her eleman yazmasinin KESIN tamsayi oldugu
      // tulpar_loop_index_proven'da kanitlandi — yani bu yazma diziyi
      // kutulayamaz ve idata askida kalamaz.
      //
      // ⚠ ICERIKTEKI KANIT DONGU BASINDA VERILIYOR, burada degil.
      // `proven_ivar` dolu olmasi zaten "govdedeki BUTUN eleman yazmalari
      // int" demek (bkz. visit_elem_write_ok); o yuzden burada RHS'i
      // yeniden sinamiyoruz. Kanit gevsetilecekse ORASI gevsetilir.
      //
      // ⚠ BU DAL, ASAGIDAKI BLOKLAR YARATILMADAN ONCE OLMAK ZORUNDA:
      // blok yaratildiktan sonraki erken `return` sonlandiricisi olmayan
      // bos bloklar birakir; LLVM 22 tolere eder, LLVM 18 SEGFAULT verir
      // (bkz. Tuzaklar 6q).
      {
        LLVMBackend::ArrShapeEntry *pshp =
            shape_lookup(backend, array_base_name(access));
        if (shape_access_proven(pshp, access->index)) {
          LLVMValueRef pidx = codegen_expression(backend, access->index);
          if (pidx) {
            LLVMValueRef pid = LLVMBuildLoad2(backend->builder,
                                              backend->ptr_type,
      ...
      if (access->name) {
        ...
        if (var_ptr) {
          target = LLVMBuildLoad2(backend->builder, backend->vm_value_type,
                                  var_ptr, access->name);
          llvm_tbaa_tag(backend, target, 0);
        }
      } else if (access->left) {
      ...
      if (target && index) {
        ...
        LLVMValueRef fn2 =
            LLVMGetBasicBlockParent(LLVMGetInsertBlock(backend->builder));
        LLVMBasicBlockRef sb_chk  = append_bb(backend, fn2, "set.chk");
        LLVMBasicBlockRef sb_fast = append_bb(backend, fn2, "set.fast");
        LLVMBasicBlockRef sb_slow = append_bb(backend, fn2, "set.slow");
        LLVMBasicBlockRef sb_done = append_bb(backend, fn2, "set.done");
        ...
        LLVMBackend::ArrShapeEntry *s_shp =
            shape_lookup(backend, array_base_name(access));
        if (s_shp) {
          LLVMBasicBlockRef sb_cached = append_bb(backend, fn2, "set.cached");
          LLVMBasicBlockRef sb_gen = append_bb(backend, fn2, "set.generic");
        ...
        // Kutulanmamis dizi: yalniz INT deger dogrudan yazilabilir. Float/bool
        // gelirse yavas yola dusuyoruz — oradaki `arr_items()` diziyi kutuya
        // cevirip anlami birebir koruyor.
        LLVMBasicBlockRef sb_uchk  = append_bb(backend, fn2, "set.ubox.chk");
        LLVMBasicBlockRef sb_ubox  = append_bb(backend, fn2, "set.unboxed");
        LLVMBasicBlockRef sb_boxed = append_bb(backend, fn2, "set.boxed");
        LLVMPositionBuilderAtEnd(backend->builder, sb_fast);
        LLVMValueRef s_id_ptr = LLVMBuildStructGEP2(
            backend->builder, backend->obj_array_type, s_objp, 5,
            "set.idata.ptr");
        LLVMValueRef s_idata = LLVMBuildLoad2(
            backend->builder, backend->ptr_type, s_id_ptr, "set.idata");
        llvm_tbaa_tag(backend, s_idata, 0);
      llvm_tbaa_tag(backend, s_idata, 0);
        LLVMValueRef s_visint = LLVMBuildICmp(
            backend->builder, LLVMIntEQ, s_vtag,
            LLVMConstInt(si32, 0 /* VM_VAL_INT */, 0), "set.visint");
        LLVMValueRef s_isubox = LLVMBuildIsNotNull(
            backend->builder, s_idata, "set.isubox");
        ...
        LLVMPositionBuilderAtEnd(backend->builder, sb_boxed);
  ...
  case AST_IF: {
    ...
    LLVMBasicBlockRef thenB =
        append_bb(backend, backend->current_function, "then");
    LLVMBasicBlockRef elseB =
        append_bb(backend, backend->current_function, "else");
    LLVMBasicBlockRef mergeB =
        append_bb(backend, backend->current_function, "merge");
  ...
  case AST_WHILE: {
    ...
    // farki, sayisal kosullarin sozdiziminden DEGIL dongu basindaki tek
    // bir sinavdan gelmesi:  v >= 0 && STEP > 0 && UB < count.
    //
    // ⚠ YALNIZ EN DIS SEVIYEDE (`for` ile ayni kural) — ve bu kez OLCUMLE:
    // ic ice acildiginda dis dongunun govdesine ikinci bir kopya giriyor ve
    // KAZANC KAYBA DONUYOR. Elek'te ic dongu tam bu bicimde ve uretilen
    // hizli govde gcc'ninkiyle KOMUT KOMUT AYNI (movq/add/cmp/jle) — ikili
    // yamalanip dogrulandi, genel surum hic kosmuyor — ama program yine de
    // 9,53 -> 10,22 ms GERILIYOR (2026-09-06, pinlenmis). En dis seviyeyle
    // sinirlandirilinca elek DEGISMIYOR (9,54) ve tek dongulu doldurma
    // 8,65 -> 8,01 kazaniyor. Ayrinti: Performance.md.
    //
    // Sekil girdisi (ileride kisit gevserse) DIS donguden gelmis olabilir;
    ...
    if (backend->shape_count > 0 &&
      ...
      if (wver) {
        ...
        LLVMValueRef num = LLVMBuildAnd(
            backend->builder,
            LLVMBuildICmp(backend->builder, LLVMIntSGE, v_val, zero, "wv.v0"),
            LLVMBuildICmp(backend->builder, LLVMIntSGT, st_val, zero, "wv.st"),
            "wv.and0");
        ok = ok ? LLVMBuildAnd(backend->builder, ok, num, "wv.and1") : num;
        for (int i = 0; i < backend->shape_count; i++) {
          LLVMValueRef cn =
              LLVMBuildLoad2(backend->builder, backend->int_type,
                             backend->shape_cache[i].count_slot, "wv.cn");
          // `v <= UB` ise en buyuk indeks UB, yani UB < count sart;
          // `v < UB` ise en buyuk indeks UB-1, yani UB <= count yeter.
        ...
        LLVMBasicBlockRef wb_fast =
            append_bb(backend, backend->current_function, "while_ver_fast");
        LLVMBasicBlockRef wb_gen =
            append_bb(backend, backend->current_function, "while_ver_gen");
        LLVMBasicBlockRef wb_done =
            append_bb(backend, backend->current_function, "while_ver_done");
        set_branch_weights(backend,
                           LLVMBuildCondBr(backend->builder, ok, wb_fast,
                                           wb_gen),
                           2000, 1);
    ...
    LLVMBasicBlockRef wb_after =
        append_bb(backend, backend->current_function, "while_after");
    codegen_while_body(backend, node, wb_after);
    LLVMPositionBuilderAtEnd(backend->builder, wb_after);
    backend->shape_count = shape_saved;
    return nullptr;
  }
  case AST_FOR_IN: {
  ...
  case AST_FOR: {
    ...
    if (ver_count > 0) {
      LLVMBasicBlockRef vb_fast =
          append_bb(backend, backend->current_function, "for_ver_fast");
      LLVMBasicBlockRef vb_gen =
          append_bb(backend, backend->current_function, "for_ver_gen");
      LLVMBasicBlockRef vb_done =
          append_bb(backend, backend->current_function, "for_ver_done");
    ...
    LLVMBasicBlockRef incrB =
        append_bb(backend, backend->current_function, "for_incr");
    LLVMBasicBlockRef exitB =
        append_bb(backend, backend->current_function, "for_exit");
  ...
  case AST_BREAK: {
    if (backend->loop_depth > 0) {
      ...
      LLVMBasicBlockRef dead = append_bb(backend, 
  ...
  case AST_TRY_CATCH: {
    ...
    LLVMBasicBlockRef tryB =
        append_bb(backend, backend->current_function, "try");
    LLVMBasicBlockRef catchB =
        append_bb(backend, backend->current_function, "catch");
    LLVMBasicBlockRef finallyB =
        node->finally_block
            ? append_bb(backend, backend->current_function, "finally")
            : nullptr;
    LLVMBasicBlockRef endB =
        append_bb(backend, backend->current_function, "try_end");
  ...
  case AST_THROW: {
    ...
    LLVMBasicBlockRef dead =
        append_bb(backend, backend->current_function, "after_throw");
    LLVMPositionBuilderAtEnd(backend->builder, dead);
    return nullptr;
  }
  ...
  if (node->body && node->body->type == AST_BLOCK && node->body->statements) {
    for (int i = 0; i < node->body->statement_count; i++) {
      ...
      if (stmt->type == AST_RETURN) {
        ...
        LLVMBasicBlockRef then_bb = append_bb(backend, func, "then");
        LLVMBasicBlockRef else_bb =
            stmt->else_branch ? append_bb(backend, func, "else") : nullptr;
        LLVMBasicBlockRef merge_bb = append_bb(backend, func, "merge");
        ...
        LLVMBasicBlockRef w_cond = append_bb(backend, func, "while.cond");
        LLVMBasicBlockRef w_body = append_bb(backend, func, "while.body");
        LLVMBasicBlockRef w_end  = append_bb(backend, func, "while.end");
        ...
        LLVMBasicBlockRef loop_cond = append_bb(backend, func, "for.cond");
        LLVMBasicBlockRef loop_body = append_bb(backend, func, "for.body");
        LLVMBasicBlockRef loop_inc = append_bb(backend, func, "for.inc");
        LLVMBasicBlockRef loop_end = append_bb(backend, func, "for.end");
          ...
          for (int j = 0; j < stmt->body->statement_count; j++) {
            ...
            if (body_stmt->type == AST_ASSIGNMENT) {
              ...
              LLVMBasicBlockRef inner_cond =
                  append_bb(backend, func, "inner.cond");
              LLVMBasicBlockRef inner_body =
                  append_bb(backend, func, "inner.body");
              LLVMBasicBlockRef inner_inc =
                  append_bb(backend, func, "inner.inc");
              LLVMBasicBlockRef inner_end =
                  append_bb(backend, func, "inner.end");
...
void codegen_func_def(LLVMBackend *backend, ASTNode_C *node) {
  ...
  bool has_captures = (cd->slots.find(node) != cd->slots.end() && !cd->slots[node].empty());
  if (has_captures) {
    int env_size = cd->slots[node].size() + 1; // +1 for parent env pointer
    LLVMValueRef alloc_args[] = {LLVMConstNull(backend->ptr_type)};
    LLVMValueRef env_obj = LLVMBuildCall2(
        backend->builder,
        LLVMGlobalGetValueType(backend->func_vm_allocate_array),
        backend->func_vm_allocate_array, alloc_args, 1, "func_alloc_env");
    backend->current_env_ptr = env_obj;

    // Slot 0: Push NULL for top-level parent env
    LLVMValueRef wrapped_parent = llvm_build_vm_val_obj(backend, LLVMConstNull(backend->ptr_type));
    LLVMValueRef val_ptr = LLVMBuildAlloca(backend->builder, backend->vm_value_type, "parent_env_val_ptr");
    LLVMBuildStore(backend->builder, wrapped_parent, val_ptr);
    LLVMValueRef val_void = LLVMBuildBitCast(backend->builder, val_ptr, backend->ptr_type, "parent_env_val_void");
    LLVMValueRef push_args[] = {LLVMConstNull(backend->ptr_type), env_obj, val_void};
...
void llvm_backend_compile(LLVMBackend *backend, ASTNode_C *node) {
  ...
  backend->current_function_node = node;
  LLVMBasicBlockRef entry = append_bb(backend, main_func, "entry");
  LLVMPositionBuilderAtEnd(backend->builder, entry);
...
// `while` govdesi — codegen_for_body'nin ikizi. Surumlemede IKI KEZ
// cagriliyor, o yuzden ayri bir fonksiyon: govde AST_BLOCK oldugu icin
// kendi kapsamini aciyor, iki yayim birbirini kirletmiyor.
static void codegen_while_body(LLVMBackend *backend, ASTNode_C *node,
                               LLVMBasicBlockRef after) {
  LLVMBasicBlockRef condB =
      append_bb(backend, backend->current_function, "cond");
  LLVMBasicBlockRef bodyB =
      append_bb(backend, backend->current_function, "body");
  LLVMBasicBlockRef exitB =
      append_bb(backend, backend->current_function, "exit");
  ...
  LLVMPositionBuilderAtEnd(backend->builder, bodyB);
  // Bu dongunun break/continue hedefleri — govdedeki AST_BREAK /
  // AST_CONTINUE nereye atlayacagini buradan ogreniyor.
  if (backend->loop_depth < 32) {
