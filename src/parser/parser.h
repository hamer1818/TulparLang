...
typedef enum {
  ...
  AST_TRY_CATCH,       // try { ... } catch (e) { ... } finally { ... }
...
typedef struct ASTNode {
  ...
  int field_count;
  struct ASTNode *
      *field_defaults; // alan varsayılan değer ifadeleri (NULL olabilir)
  ...
  // Try-Catch için
  struct ASTNode *try_block;     // try bloğu
  struct ASTNode *catch_block;   // catch bloğu
  struct ASTNode *finally_block; // finally bloğu (opsiyonel)
  char *catch_var;               // catch(e) -> "e"
  struct ASTNode *throw_expr;    // throw ifadesi
