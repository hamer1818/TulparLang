...
struct FunctionCall {
    ...
    std::unique_ptr<ASTNode> callee;   // For calling arbitrary expressions (indirect calls)
    SourceLocation loc;
    
    FunctionCall(const std::string& n,
                 std::vector<std::unique_ptr<ASTNode>> args,
                 SourceLocation l)
        : name(n), arguments(std::move(args)), receiver(nullptr), callee(nullptr), loc(l) {}

    FunctionCall(std::unique_ptr<ASTNode> c,
                 std::vector<std::unique_ptr<ASTNode>> args,
                 SourceLocation l)
        : name(""), arguments(std::move(args)), receiver(nullptr), callee(std::move(c)), loc(l) {}
};
