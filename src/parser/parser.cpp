...
// shader alt kumesi GLSL'e ceviriliyor (motor deposu: docs/FAZ8.md, Secenek C) ve
...
std::unique_ptr<ASTNode> Parser::parse_primary() {
    ...
    // ayrisamiyordu (motor deposu: docs/FAZ8.md T19). Her dilde bulunan bu yazim
    ...
    if (check(TOKEN_LPAREN)) {
        ...
        while (scan_pos < tokens_.size() && tokens_[scan_pos].type() != TOKEN_EOF) {
            if (tokens_[scan_pos].type() == TOKEN_LPAREN) {
                paren_nesting++;
            } else if (tokens_[scan_pos].type() == TOKEN_RPAREN) {
                paren_nesting--;
                if (paren_nesting == 0) {
                    if (scan_pos + 1 < tokens_.size() && tokens_[scan_pos + 1].type() == TOKEN_FAT_ARROW) {
                        is_lambda = true;
                    }
                    break;
                }
        ...
        if (is_lambda) {
            SourceLocation lam_loc(current().line(), current().column());
            advance(); // consume '('
            std::vector<Parameter> parameters;
            if (!check(TOKEN_RPAREN)) {
                do {
...
std::unique_ptr<ASTNode> Parser::parse_call(std::unique_ptr<ASTNode> callee) {
    ...
    if (auto* id = std::get_if<Identifier>(&callee->value)) {
        return std::make_unique<ASTNode>(
            FunctionCall(id->name, std::move(arguments), loc)
        );
    } else {
        return std::make_unique<ASTNode>(
            FunctionCall(std::move(callee), std::move(arguments), loc)
        );
    }
}
...
DataType Parser::parse_type() {
    ...
    // seydir. Bu BILEREK boyle: amac shader alt kumesinin (motor deposu:
    // docs/FAZ8.md T8) ayrisabilmesi; gercek sabit-boy semantigi kutusuz
    // struct/yerlesim isiyle (PLAN §11) birlikte gelir. Boyut SESSIZCE
    // degil, bu yorumla ve raporla dusuruluyor.
...
static ASTNode_C* convert_ast_node(const ASTNode& node) {
    ...
    std::visit([&](const auto& n) {
        ...
        if constexpr (std::is_same_v<T, IntLiteral>) {
            ...
            out->callee = convert_ast_node_ptr(n.callee);
            set_loc(out, n.loc);
...
static void ast_node_free_recursive(ASTNode_C* node) {
    ...
    ast_node_free_recursive(node->callee);
