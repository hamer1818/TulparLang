  ...
  while (running) {
    ...
    if (ImGui::Begin(kPanelGorunumLabel)) {
      ...
      if (ImGui::BeginPopup("ViewportOptionsMenu")) {
        ...
        if (ImGui::MenuItem( ICON_MD_CENTER_FOCUS_STRONG " Se\xC3\xA7ili Varl\xC4\xB1\xC4\x9Fa Odaklan")) {
          const int32_t s0 = st.sel.primary();
          if (s0 >= 0 && s0 < (int32_t)st.scene.entity_count) {
            static content::SceneBounds fb[content::kSceneMaxEntities];
            const uint32_t nb = entity_world_bounds(st, phys, fb);
    ...
    if (ImGui::Begin(kPanelSahneLabel)) {
      ...
      if (ImGui::BeginPopupContextWindow("SahnePanelMenu", ImGuiPopupFlags_MouseButtonRight | ImGuiPopupFlags_NoOpenOverItems)) {
        ...
        if (ImGui::MenuItem("Yeni Geli\xC5\x9Fmi\xC5\x9F Varl\xC4\xB1k Taray\xC4\xB1c\xC4\xB1", nullptr, &st.show_asset_browser)) {}
        ...
        if (has_terrain) {
          ...
          if (component_header(ICON_MD_LANDSCAPE, "Arazi (Terrain)", nullptr, &rem, true, Tone::AxisY)) {
            if (prop_begin("terrain")) {
              ...
              ImGui::Separator();
              if (ImGui::Button(ICON_MD_SAVE " Prefab Olarak Kaydet", ImVec2(-1, 0))) {
                // TODO: Prefab kaydetme mantigi
                std::snprintf(st.hist.status, sizeof(st.hist.status), "Prefab 'assets/terrain.prefab' olarak diske kaydedildi.");
              }
              prop_end();
            }
            component_end();
        ...
        if (has_skybox) {
          ...
          if (component_header(ICON_MD_CLOUD, "G\xC3\xB6ky\xC3\xBCz\xC3\xBC (Skybox)", nullptr, &rem, true, Tone::AxisZ)) {
            if (prop_begin("skybox")) {
              ImGui::Text("HDRI K\xC3\xBCp Haritas\xC4\xB1:");
              ImGui::Button("hdri_sky.hdr", ImVec2(-1, 0));
              prop_end();
            }
            component_end();
          }
    ...
    if (ImGui::Begin(kPanelDunyaLabel)) {
      ...
      if (prop_begin("navmesh")) {
        ...
        if (ImGui::Button(ICON_MD_DIRECTIONS_RUN " NavMesh Pi\xC5\x9Fir", ImVec2(-1, 0))) {
          console_log(ConsoleLevel::Bilgi, "engine", "NavMesh (Recast) in\xC5\x9Fa edildi: 0 poligon, 0 k\xC3\xB6\xC5\x9Fe.");
        }
        prop_end();
      }
      ...
      if (prop_begin("ssao")) {
        prop_help("Screen Space Ambient Occlusion (SSAO): K\xC3\xB6\xC5\x9Felerin ve dar alanlar\xC4\xB1n i\xC3\xA7ini karartarak derinlik hissini art\xC4\xB1r\xC4\xB1r.");
    ...
    if (st.show_terrain_editor) {
      ...
      if (ImGui::Begin(ICON_MD_FORMAT_PAINT " Arazi (Terrain) F\xC4\xB1r\xC3\xA7" "as\xC4\xB1", &st.show_terrain_editor)) {
        ImGui::TextWrapped("Sol t\xC4\xB1k ile arazi \xC3\xBCzerine se\xC3\xA7ili varl\xC4\xB1\xC4\x9F\xC4\xB1 (a\xC4\x9Fa\xC3\xA7/ot) ekebilirsiniz.");
        ...
        ImGui::RadioButton("Bitki/A\xC4\x9Fa\xC3\xA7 Ek (Foliage)", &brush_mode, 1);
