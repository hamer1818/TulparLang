...
Windows CI runs the same evidence chain as Linux: `build.sh test` (every example), `build.sh suites`, the typeinfer gate, then the Inno Setup installer. Only the Python audits are skipped — they're platform-independent and the Linux job covers them. Measured 2026-09-21: the build-only Windows job took 3m35s against Linux's 13m46s, so there was room for the tests and the "too expensive" argument didn't survive contact with the numbers.

Releases carry `tulpar-setup-windows-x64.exe` (per-user GUI installer, no admin prompt — `installer/tulpar.iss`, Inno Setup 6) alongside the portable zip. (History: native Windows was dropped in 3.13.0 — CI job, installer and all — but CMakeLists/`build.sh` kept and later regained their MinGW support, so the repo could build Windows while nothing measured it. The first restored run immediately caught a months-old compile break in `aot_pipeline.cpp`. There is still no `build.bat`/`build.ps1`.)
...
Then rebuild **both** `tulpar` and `tulpar_runtime`, and refresh the repo-root `libtulpar_runtime.a` if AOT link probing needs it. **If the builtin is a tame one, also refresh the prebuilt archives** — `wasm/build_tame_web.sh` (emsdk is vendored) and, with an NDK, `android/build_tame_android.sh`. Skipping this breaks every `--target=web` / `--target=android` build with `undefined symbol` while the desktop build, the suites and the examples all stay green; `tests/dist_archive_audit.py` (run by `./build.sh suites`) names the missing symbols. **Changing an existing builtin's signature is a different and nastier case than adding one** (measured 2026-09-16 on `aot_input`): the symbol is still present, so the symbol audit stays green, and `wasm-ld` downgrades the mismatch to a *warning* and links anyway — the break only surfaces in the browser at the call. Refresh the archives in the **same** change, never a later one; the audit's `check_archive_freshness()` (archive older than the runtime sources the driver itself lists) is what catches this class (Tuzaklar 8aq). Note also that `android/build_tame_android.sh` deletes only its own outputs rather than the whole `android/dist/<abi>/` directory — a habit from when the engine archives shared that directory (Tuzaklar 8ar, now in the engine repo). Pure-Tulpar helpers (most of Wings/ORM) don't need any of this — they live in `lib/*.tpr` and only require a `./build.sh clean` to re-embed.
