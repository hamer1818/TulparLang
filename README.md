...
<!-- BENCH:META START -->
> _Auto-updated by CI on every push to `main`. Last run: **2026-05-12T19:30:46Z** UTC · commit [`625dc41`](../../commit/625dc4148d356eef8755404bf190bcb4057de20a) · runner `Linux` · `Intel(R) Xeon(R) Platinum 8370C CPU @ 2.80GHz` (4 CPUs). Methodology: [benchmarks/CI.md](benchmarks/CI.md)._
<!-- BENCH:META END -->
...
|---|---:|---:|---:|---:|---:|---:|
| loopsum (ms) | **1.5** | 0.5 | 0.6 | 4.9 | 39.7 | 829.2 |
| fib(35) (ms) | **27.5** | 16.1 | 25.6 | 50.7 | 121.4 | 1147.6 |

Tulpar AOT lands at **1.71×–3.00× of C (gcc -O2)** on these microbenchmarks (i.e. C-comparable, with a small multiplicative gap), **4.4–26.5× faster than Node.js**, and **42–553× faster than Python**.
<!-- BENCH:CPU_TABLE END -->
...
|---|---|---:|---:|
| **Tulpar listen** | single thread, one request at a time | 24 046 | **1.73× faster** |
| **Tulpar listen_evented** | single thread, poll()-multiplexed | 23 114 | **1.66× faster** |
| **Tulpar listen_async** | OS thread spawned per connection | 23 074 | **1.66× faster** |
| **Tulpar listen_pool** | worker pool sized to host CPU count, sharing accept() | 23 057 | **1.66× faster** |
| **Tulpar evented + cache** | evented + wire-byte cache for cached_get routes | 22 020 | **1.58× faster** |
| Node.js http | single-thread event loop | 13 924 | _(baseline)_ |
| Python ThreadingHTTP | OS thread spawned per request | 98 | 142.08× slower |

5 of the Tulpar Wings listeners at this concurrency beat Node.js' built-in `http`, by **1.58×–1.73×** depending on scheduling model.
...
| Server | Scheduling model | req/sec | vs Node.js |
|---|---|---:|---:|
| **Tulpar evented + cache** | evented + wire-byte cache for cached_get routes | 25 765 | **1.11× faster** |
| **Tulpar listen_async** | OS thread spawned per connection | 25 416 | **1.09× faster** |
| **Tulpar listen_evented** | single thread, poll()-multiplexed | 25 270 | **1.09× faster** |
| **Tulpar listen_pool** | worker pool sized to host CPU count, sharing accept() | 23 598 | **1.02× faster** |
| **Tulpar listen** | single thread, one request at a time | 23 412 | **1.01× faster** |
| Node.js http | single-thread event loop | 23 248 | _(baseline)_ |
| Python ThreadingHTTP | OS thread spawned per request | 391 | 59.46× slower |

5 of the Tulpar Wings listeners under this load beat Node.js' built-in `http`, by **1.01×–1.11×** depending on scheduling model.
