...
<!-- BENCH:META START -->
> _Auto-updated by CI on every push to `main`. Last run: **2026-05-12T18:34:52Z** UTC · commit [`315c5e3`](../../commit/315c5e3100bce9c643ac50219b9cd7762fe97e7c) · runner `Linux` · `AMD EPYC 9V74 80-Core Processor` (4 CPUs). Methodology: [benchmarks/CI.md](benchmarks/CI.md)._
<!-- BENCH:META END -->
...
|---|---:|---:|---:|---:|---:|---:|
| loopsum (ms) | **2.3** | 0.7 | 0.8 | 4.7 | 40.4 | 1068.8 |
| fib(35) (ms) | **31.6** | 17.1 | 32.4 | 57.8 | 142.0 | 1258.1 |

Tulpar AOT lands at **1.85×–3.29× of C (gcc -O2)** on these microbenchmarks (i.e. C-comparable, with a small multiplicative gap), **4.5–17.6× faster than Node.js**, and **40–465× faster than Python**.
<!-- BENCH:CPU_TABLE END -->
...
|---|---|---:|---:|
| **Tulpar listen** | single thread, one request at a time | 23 316 | **2.04× faster** |
| **Tulpar listen_evented** | single thread, poll()-multiplexed | 23 059 | **2.01× faster** |
| **Tulpar evented + cache** | evented + wire-byte cache for cached_get routes | 22 979 | **2.01× faster** |
| **Tulpar listen_async** | OS thread spawned per connection | 22 488 | **1.96× faster** |
| **Tulpar listen_pool** | worker pool sized to host CPU count, sharing accept() | 22 376 | **1.95× faster** |
| Node.js http | single-thread event loop | 11 453 | _(baseline)_ |
| Python ThreadingHTTP | OS thread spawned per request | 98 | 116.87× slower |

5 of the Tulpar Wings listeners at this concurrency beat Node.js' built-in `http`, by **1.95×–2.04×** depending on scheduling model.
...
| Server | Scheduling model | req/sec | vs Node.js |
|---|---|---:|---:|
| **Tulpar listen** | single thread, one request at a time | 23 456 | **1.54× faster** |
| **Tulpar listen_pool** | worker pool sized to host CPU count, sharing accept() | 22 236 | **1.46× faster** |
| **Tulpar evented + cache** | evented + wire-byte cache for cached_get routes | 19 596 | **1.29× faster** |
| **Tulpar listen_async** | OS thread spawned per connection | 19 588 | **1.29× faster** |
| **Tulpar listen_evented** | single thread, poll()-multiplexed | 19 313 | **1.27× faster** |
| Node.js http | single-thread event loop | 15 202 | _(baseline)_ |
| Python ThreadingHTTP | OS thread spawned per request | 391 | 38.88× slower |

5 of the Tulpar Wings listeners under this load beat Node.js' built-in `http`, by **1.27×–1.54×** depending on scheduling model.
