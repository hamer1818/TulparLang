...
> ## ⚠️ BU TABLO GÜVENİLİR DEĞİL — `benchmarks/fair/` kullanın
>
> Denetlendi (2026-09-02) ve üç ayrı kusuru bulundu; üçü de sonucu geçersiz kılıyor:
>
> 1. **C ve Rust hiç döngü koşmuyordu.** Yalnız Tulpar iş yükünü ortamdan
>    okuyordu; ötekiler derleme-zamanı sabiti alıyordu ve `gcc -O2` / `rustc -O3`
>    `loopsum`u kapalı forma katlıyordu — `objdump` ile doğrulandı, `main` içinde
>    sıfır atlama komutu.
> 2. **Aynı algoritma değildi.** `sieve`de C `char*`, Tulpar `json`+push;
>    `struct_array_push`ta C tek `malloc`, Tulpar büyüyen dizi.
> 3. **"best of 1"** ve bütün süreler 22–60 ms — ölçülenin çoğu süreç başlatma.
>    Ayrıca **silinmiş VM** satırı (3.13.0'da kaldırıldı) ve bozuk oran formülü
>    (`tak`ta Tulpar Python'dan yavaşken "0× faster" yazıyor) duruyor.
>
