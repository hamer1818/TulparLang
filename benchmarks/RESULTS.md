...
**Run:** `2026-05-13T15:43:34Z` UTC · commit `f9463eb` · runner `Linux` `AMD EPYC 9V74 80-Core Processor` · best of 3.
...
| Python | 701.5 | 981.7 |
| Rust (-O3) | 0.7 | 25.2 |
| **Tulpar AOT (LLVM)** | 1.6 | 26.1 |
| **Tulpar VM** | 120.8 | 447.2 |

**Speed ratios** — how Tulpar AOT compares to each reference language:

| Workload | vs C (gcc -O2) | vs Node.js | vs Python |
|---|---:|---:|---:|
| loopsum | 2.67× of C | **20.2× faster** | **438× faster** |
| fib | 1.96× of C | **4.2× faster** | **38× faster** |
...
| Python ThreadingHTTP | OS thread spawned per request | 98 | 155× slower |
...
| Python ThreadingHTTP | OS thread spawned per request | 391 | 54× slower |
