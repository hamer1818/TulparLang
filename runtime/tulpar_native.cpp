...
TulparString tulpar_string_concat(TulparString a, TulparString b) {
  ...
  char *result = static_cast<char *>(malloc(len_a + len_b + 1));
...
TulparString tulpar_string_substring(TulparString s, TulparInt start, TulparInt end) {
  ...
  char *result = static_cast<char *>(malloc(sub_len + 1));
...
TulparArrayInt* tulpar_array_int_new(void) {
  TulparArrayInt *arr = static_cast<TulparArrayInt *>(malloc(sizeof(TulparArrayInt)));
  arr->capacity = 8;
  arr->count = 0;
  arr->data = static_cast<TulparInt *>(malloc(sizeof(TulparInt) * arr->capacity));
  return arr;
}
...
void tulpar_array_int_push(TulparArrayInt *arr, TulparInt val) {
  ...
  if (arr->count >= arr->capacity) {
    arr->capacity *= 2;
    arr->data = static_cast<TulparInt *>(realloc(arr->data, sizeof(TulparInt) * arr->capacity));
  }
...
TulparArrayFloat* tulpar_array_float_new(void) {
  TulparArrayFloat *arr = static_cast<TulparArrayFloat *>(malloc(sizeof(TulparArrayFloat)));
  arr->capacity = 8;
  arr->count = 0;
  arr->data = static_cast<TulparFloat *>(malloc(sizeof(TulparFloat) * arr->capacity));
  return arr;
}
...
void tulpar_array_float_push(TulparArrayFloat *arr, TulparFloat val) {
  ...
  if (arr->count >= arr->capacity) {
    arr->capacity *= 2;
    arr->data = static_cast<TulparFloat *>(realloc(arr->data, sizeof(TulparFloat) * arr->capacity));
  }
  arr->data[arr->count++] = val;
}
...
TulparArrayStr* tulpar_array_str_new(void) {
  TulparArrayStr *arr = static_cast<TulparArrayStr *>(malloc(sizeof(TulparArrayStr)));
  arr->capacity = 8;
  arr->count = 0;
  arr->data = static_cast<TulparString *>(malloc(sizeof(TulparString) * arr->capacity));
  return arr;
}
...
void tulpar_array_str_push(TulparArrayStr *arr, TulparString val) {
  ...
  if (arr->count >= arr->capacity) {
    arr->capacity *= 2;
    arr->data = static_cast<TulparString *>(realloc(arr->data, sizeof(TulparString) * arr->capacity));
  }
  arr->data[arr->count++] = tulpar_string_new(val);
}

TulparString tulpar_array_str_get(TulparArrayStr *arr, TulparInt index) {
  if (!arr || index < 0 || index >= arr->count) return nullptr;
  return arr->data[index];
}
...
TulparInt tulpar_string_to_int(TulparString s) {
  if (!s) return 0;
  return (TulparInt)strtoll(s, nullptr, 10);
}
